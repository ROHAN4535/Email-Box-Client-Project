...
const Inbox = () => {
  ...
  const clickEmailHandler = async (item) => {
    ...
    try {
      const resEmail = await fetch(
        `https://email-box-client-f9178-default-rtdb.firebaseio.com/${email}/recievedemails/${item[0]}.json`,
        {
          method: "PUT",
          body: JSON.stringify({
            id: item[1].id,
  ...
  const clickDeleteHandler = async (deleteItem) => {
    ...
    try {
      await fetch(
        `https://email-box-client-f9178-default-rtdb.firebaseio.com/${email}/recievedemails/${deleteItem[0]}.json`,
        { method: "DELETE" }
      );
    } catch (err) {
      console.log(err);
